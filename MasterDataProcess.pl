...
sub main() {
   ...
   while ( $myReturn ) {
      ...
      print "6. Delete Summary & Filenames Sheets? ",
        $Delete_Summary_Filename_Sheets;
...
sub ProcessTFTAutoprobeData() {
   ...
   %Environment{ 'Test_Descriptor_Keys_File' } =
     prompt( "Enter Test Descriptor Keys File or \"(compute)\"" );
   ...
   say "Test Descriptor Keys File: ",
     %Environment{ 'Test_Descriptor_Keys_File' };
   say "Test Name For Processing: ",
     %Environment{ 'Test_Name_For_Processing' };
   ...
   say "Delete Summary Filenames?: ",
     %Environment{ 'Delete_Summary_Filenames_Sheets' };

   if (
      ( my $continue = prompt( "Continue?" ) ~= /no|n|No/ )
      {
         return ( 1 );
      }
...
# First, open the TestDescriptorKeys Workbook and switch to the specified Worksheet
         ...
         my $workbook =
           $parser->parse( %Environment{ 'Test_Descriptor_Keys_File' } );
